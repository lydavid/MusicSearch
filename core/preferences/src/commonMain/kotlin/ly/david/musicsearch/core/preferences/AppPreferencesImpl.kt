...
internal class AppPreferencesImpl(
    ...
    // region Place
    private val placeSortOptionPreference = stringPreferencesKey(AppPreferencesKey.PLACE_SORT_OPTION.name)

    override val placeSortOption: Flow<PlaceSortOption>
        get() = preferencesDataStore.data
            .map {
                it[placeSortOptionPreference].toEnumOrDefault(PlaceSortOption.InsertedAscending)
            }
            .distinctUntilChanged()
    ...
    override fun setPlaceSortOption(sort: PlaceSortOption) {
        coroutineScope.launch {
            preferencesDataStore.edit {
                it[placeSortOptionPreference] = sort.name
            }
        }
    }
    // endregion
    ...
    // region Series
    private val seriesSortOptionPreference = stringPreferencesKey(AppPreferencesKey.SERIES_SORT_OPTION.name)

    override val seriesSortOption: Flow<SeriesSortOption>
        get() = preferencesDataStore.data
            .map {
                it[seriesSortOptionPreference].toEnumOrDefault(SeriesSortOption.InsertedAscending)
            }
            .distinctUntilChanged()
    ...
    override fun setSeriesSortOption(sort: SeriesSortOption) {
        coroutineScope.launch {
            preferencesDataStore.edit {
                it[seriesSortOptionPreference] = sort.name
            }
        }
    }
    // endregion
